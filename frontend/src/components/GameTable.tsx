...
const GameTable: React.FC<GameTableProps> = ({
    ...
    return (
        <div className="relative w-full h-screen bg-[#1c3a26] overflow-hidden flex flex-col justify-between p-4 isolate">
            ...
            {/* Dynamic Opponents Row (Top/Sides) */}
            <div className="relative z-10 flex-1 flex flex-col justify-start pt-16 md:pt-20 px-4 md:px-12 pointer-events-none">
                <div className="flex flex-wrap justify-center gap-4 md:gap-8 w-full">
                    {opponents.map((opp) => (
                        <div key={opp.id} className="text-center text-white p-2 md:p-3 bg-black/40 rounded-xl border border-white/10 backdrop-blur-sm min-w-[80px] md:min-w-[120px]">
                            <p className="font-bold text-xs md:text-base truncate">{opp.name}</p>
                            <p className="text-[10px] md:text-xs text-orange-400 font-mono mt-1">💩 {opp.chili} Kg</p>
                            {opp.status === 'ready_to_pass' && (
                                <p className="text-[9px] md:text-[10px] text-green-400 mt-1 uppercase tracking-wider">Pronto</p>
                            )}
                        </div>
                    ))}
                </div>
